...
def run_bo(Z_obs, Y_obs, bounds, n_iter, latent_size, model, tk, max_len, device, acquisition_samples=1000):
    """
    ...
    for i in range(Y_obs.shape[1]):
        ...
        # Y_target = Y_obs[:, i].reshape(-1, 1) + np.random.randn(Y_obs.shape[0], 1) * 1e-4
        Y_target = Y_obs[:, i].reshape(-1, 1)
        try:
            gp = GPy.models.GPRegression(Z_obs, Y_target, kernel)
            gp.optimize(messages=False, max_iters=200) 
            gp.optimize_restarts(num_restarts=3, verbose=False)
            print(f"  GP for Objective {i+1} trained. Final Params: {gp}")
        except np.linalg.LinAlgError:
             print(f"  Warning: GP Optimization failed for Objective {i+1} due to LinAlgError. Using default kernel params.")
             # Fallback to unoptimized GP
             gp = GPy.models.GPRegression(Z_obs, Y_target, kernel)
        except Exception as e:
            print(f"  Warning: An unexpected error occurred during GP optimization for Objective {i+1}: {e}")
            gp = GPy.models.GPRegression(Z_obs, Y_target, kernel) # Fallback

        gp_models.append(gp)
        print(f"  GP Model {i+1} Summary:\n{gp}")
    ...
    for bo_step in tqdm(range(n_iter), desc="  BO Steps"):
        ...
        # --- Simplified EHVI Approximation ---
        # Calculate the current Pareto front based on *observed* data
        pareto_idx_obs = get_pareto_front(Y_obs)
        current_pareto_Y = Y_obs[pareto_idx_obs]

        if current_pareto_Y.shape[0] == 0: # Handle empty Pareto front case
             print("Warning: Current Pareto front is empty. Selecting based on predicted means.")
             # Select the point with the best predicted mean product or sum
             # Simple approach: maximize sum of predicted means
             acquisition_scores = np.sum(Y_pred_mean, axis=1)

        else:
            # Reference point (approximation - nadir point of current Pareto)
            # ref_point = np.min(current_pareto_Y, axis=0) - 0.1 # Slightly worse than worst Pareto point

            acquisition_scores = []
            for k in range(acquisition_samples):
                y_pred_k = Y_pred_mean[k]
                # Calculate the approximate improvement in hypervolume if y_pred_k is added
                # This is a *very* rough approximation of EHVI.
                # Consider potential new front with the predicted point
                ...
                # Simple metric: product of ranges on the *new* predicted front
                # This encourages spreading out the front.
                if new_pareto_Y.shape[0] > 1:
                     score = np.prod(np.max(new_pareto_Y, axis=0) - np.min(new_pareto_Y, axis=0))
                elif new_pareto_Y.shape[0] == 1: # Single point front
                     # How much does this point improve over the existing front (if exists)?
                     # Use distance from origin or similar simple metric if no existing front
                ...
                else: # Should not happen if y_pred_k is included
                     score = 0

                # Add exploration bonus based on variance (optional, simple UCB-like idea)
                # exploration_bonus = 0.1 * np.sum(np.sqrt(Y_pred_var[k]))
                # score += exploration_bonus

                acquisition_scores.append(score)

        # Select the candidate with the highest acquisition score
        best_idx = np.argmax(acquisition_scores)
        z_next = Z_candidates[best_idx:best_idx+1] # Keep as 2D array

        # Evaluate the selected point using the *actual* objective functions
        # print(f"  Evaluating selected Z: {z_next}")
        y_next, smiles_next_all, smiles_next_valid, z_next_valid = compute_targets(z_next, model, tk, max_len, device)
        # print(f"  Evaluated Y: {y_next}, Valid: {len(smiles_next_valid)>0}")

        # Add the new observation(s) to the dataset
        # Only add if the evaluation was successful (e.g., SMILES was valid)
        # If using z_next_valid, ensure y_next corresponds correctly (it should if compute_targets returns matching valid_z)
        if len(smiles_next_valid) > 0:
            Z_obs = np.vstack([Z_obs, z_next_valid]) # Use z corresponding to valid smiles
            valid_y_next_mask = y_next[:, 1] > 0.0
            Y_obs = np.vstack([Y_obs, y_next[valid_y_next_mask]])
            Z_new_list.extend(z_next_valid)
            Y_new_list.extend(y_next[valid_y_next_mask]) # Filter y_next based on validity score > 0
            smiles_new_all.extend(smiles_next_all) # Keep all generated for record
            smiles_new_valid.extend(smiles_next_valid)

            # Update GPs with the new point for the *next* BO step in this iteration
            print("  Updating GP models with new point...")
            for i, gp in enumerate(gp_models):
                try:
                    gp.set_XY(Z_obs, Y_obs[:, i].reshape(-1, 1))
                    gp.optimize(messages=False, max_iters=100) # Quicker update
                except Exception as e:
                    print(f"  Warning: GP Update failed for Objective {i+1}: {e}. Model might be inaccurate.")

        else:
            #  print(f"  Skipping update for invalid SMILES: {smiles_next_all}")
            pass


    return Z_obs, Y_obs, np.array(Z_new_list), np.array(Y_new_list), smiles_new_all, smiles_new_valid
...
def update_decoder_idl(model, Z_elite, smiles_elite, tk, max_len, device, idl_epochs=5, idl_lr=1e-4, lambda_kl=0.01):
    ...
    model.train() # Set model to training mode for IDL
    ...
    if Z_elite.shape[0] < 2:
        ...
        std_target_np[std_target_np < 1e-6] = 1e-6 # Avoid zero std dev
        ...
        mu_target_np = Z_elite.mean(axis=0)
        cov_target_np = np.cov(Z_elite.T)
        # Add small value to diagonal for numerical stability
        cov_target_np += np.eye(Z_elite.shape[1]) * 1e-6
        try:
            # Use only diagonal elements for Normal distribution KL
            std_target_np = np.sqrt(np.diag(cov_target_np))
            std_target_np[std_target_np < 1e-6] = 1e-6 # Avoid zero std dev
            mu_target = torch.tensor(mu_target_np, dtype=torch.float32, device=device)
            std_target = torch.tensor(std_target_np, dtype=torch.float32, device=device)
            print(f"  Target mu shape: {mu_target.shape}, Target std shape: {std_target.shape}")
        except np.linalg.LinAlgError:
            print("  Warning: Covariance matrix calculation failed in IDL. Using diagonal std dev.")
            mu_target_np = Z_elite.mean(axis=0)
            std_target_np = Z_elite.std(axis=0)
            std_target_np[std_target_np < 1e-6] = 1e-6 # Avoid zero std dev
            mu_target = torch.tensor(mu_target_np, dtype=torch.float32, device=device)
    ...
    for epoch in range(idl_epochs):
        ...
        for batch in elite_loader:
            ...
            # KL between encoded distribution q(z|x_elite) and target distribution p_target(z)
            q_dist = torch.distributions.Normal(mu_enc, torch.exp(0.5 * logvar_enc))
            # Ensure dimensions match for KL divergence calculation
            # Target dist might be defined on the whole elite set, Q needs to match batch stats
            # We sum KL over the batch dimension
            kl_loss = torch.distributions.kl_divergence(q_dist, target_dist).sum()
            # Average KL per sample in batch
            kl_loss = kl_loss / x_batch.size(0)

            # Total Loss
            loss = recon_loss / x_batch.size(0) + lambda_kl * kl_loss # Average recon loss per sample
...
def optimize_molecules(args):
    ...
    model = VAE_Autoencoder(vocab_size, args.embedding_dim, args.hidden_size, args.latent_size,
    ...
    if Z_init_valid.shape[0] < 2: # Need at least 2 points for BO
        print("Error: Not enough valid initial points generated to start BO. Try increasing --bo_initial_points or checking the VAE model.")
        return

    # Use only valid points for the initial BO dataset
    Z_obs = Z_init_valid
    Y_obs = Y_init[np.all(Y_init != [-10.0, 0.0], axis=1)] # Filter out invalid scores
    ...
    bounds = (lb, ub)
    print(f"Latent space bounds (min/max derived from initial valid points): {bounds}")


    # --- Main BO-IDL Loop ---
    all_smiles_generated = list(smiles_init_all) # Keep track of all attempted SMILES
    all_smiles_valid = list(smiles_init_valid) # Keep track of all valid SMILES
    hypervolumes = []
    pareto_fronts_Y = [] # Store Pareto fronts over iterations
    ...
    for iteration in range(args.bo_idl_iterations):
        ...
        print(f"BO finished. Total observed points: {Z_obs.shape[0]}")
        ...
        # Filter out any potentially invalid smiles from the elite set if compute_targets logic changes
    ...
    with open(os.path.join(results_dir, 'final_pareto_smiles.txt'), 'w') as f:
        for smi, y in zip(final_smiles_pareto, final_Y_pareto):
             ...
             if mol: # Double check validity before saving final list
                 f.write(f"{smi}\t{-y[0]:.4f}\t{y[1]:.4f}\n") # Save with SAS (non-negated), QED
